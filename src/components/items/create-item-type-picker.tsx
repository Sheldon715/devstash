    ...
    <section className="space-y-3">
      ...
      <div className="relative inline-block max-w-full">
        <button
          ...
          disabled={disabled}
          onClick={onToggle}
          className="inline-flex h-10 max-w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-left text-sm font-medium text-zinc-100 outline-none transition-colors hover:border-white/18 hover:bg-white/[0.06] focus:border-sky-300/35 focus:bg-white/[0.06] disabled:cursor-not-allowed disabled:opacity-60"
        >
          ...
          />
          <span className="min-w-0 truncate">{selectedTypeOption.label}</span>
        ...
        {isOpen ? (
          <div
            role="listbox"
            className="create-type-menu-enter absolute top-[calc(100%+0.5rem)] left-0 z-20 w-max min-w-full max-w-[min(20rem,calc(100vw-3rem))] rounded-2xl border border-white/10 bg-[#0b0d12] p-1.5 shadow-[0_18px_60px_rgba(0,0,0,0.45)]"
          >
